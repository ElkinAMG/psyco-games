...
window.onload = function () {
  ...
  var days = [
    {
      ...
      score: 15,
      instruction:
        "<p>Estás a punto de entrar a una aventura submarina<br>Hay un total 5 dias donde estarás pescando valores y <br> esquivando antivalores y otros obstaculos <br>Necesitas atrapar ciertos peces dorados que estarán<br>representando los bonitos valores que nos acompañan <br>así como en la vida recuerda atrapar esos valores que nos <br> hacen mejor persona, como <span id='valores'>la honestidad, responsabilidad, solidaridad <br> respeto, amor, gratitud, tolerancia, perdón </span> y muchos más!<br>¡Recuerda esquivar esos antivalores que serán de color morado!</p><p>Feliz pesca!</p>",
    },
    {
      day: 1,
      score: 20,
      instruction:
        "Excelente, eres muy bueno en esto!<br>Ahora estas por enfrentarte a los antivalores, ten cuidado!<br> Y recuerda, son los malos y morados!",
    },
    {
      day: 2,
      score: 25,
      instruction:
        "Bien hecho! haz pasado de día! <br>Por desgracia, Hay mucha basura encontrada en los oceanos.<br>Hay una penalizacion si las atrapas así que ten cuidado!<br> Buena suerte y evita esa basura!",
    },
    {
      day: 3,
      score: 30,
      instruction:
        "Bien hecho! <br>Espera, ¿Que eso eso que se ve allá?<br> ¡Oh no! Medusas, han invadido esta parte del oceano.<br>Te van a inmovilizar un par de segundos si las atrapas<br>Así que ten cuidado y esquivalas!",
    },
    {
      day: 4,
      score: 35,
      instruction:
        "Tiburones aquí!<br>Habrás perdido de inmediato si te atrapan!<br>Así que ten cuidado, que la victoria ya es casi tuya!",
    },
  ];
  ...
  function createItems() {
    createTimer();
    day++;
    gameDay.innerText = "Día 0" + day;
    gameGoal.innerText = `Meta: ${currentScore}/${days[day - 1].score}`;
    ...
    switch (day) {
      ...
      case 2:
        createFishInterval = setInterval(createFish, 250);
        createRareFishInterval = setInterval(createRareFish, 400);
        break;

      case 3:
        createFishInterval = setInterval(createFish, 250);
        createRareFishInterval = setInterval(createRareFish, 900);
    ...
    if (!item.classList.contains("trash")) {
      ...
      if (randomNum % 2 === 0) {
        ...
        setInterval(function () {
          let speedFactor = 0.8; // Puedes ajustar este valor según sea necesario

          if (item.classList.contains("fish")) {
            leftPos += 45 * speedFactor; // Reducir la velocidad
          } else if (item.classList.contains("rare-fish")) {
            leftPos += 65 * speedFactor; // Reducir la velocidad
          } else if (item.classList.contains("jellyfish")) {
            leftPos += 5 * speedFactor; // Reducir la velocidad
          } else if (item.classList.contains("shark")) {
            leftPos += 15 * speedFactor; // Reducir la velocidad
            // También puedes ajustar la velocidad vertical si es necesario
            if (topPos > mousePosition.y) {
              topPos -= 10 * speedFactor;
            } else {
              topPos += 10 * speedFactor;
            }
          }
      ...
      //right side
        ...
        setInterval(function () {
          ...
          // Modificar las coordenadas left y top según el tipo de elemento
          if (item.classList.contains("fish")) {
            leftPos -= 45 * speedFactor; // Reducir la velocidad
          } else if (item.classList.contains("rare-fish")) {
            leftPos -= 65 * speedFactor; // Reducir la velocidad
    ...
    if (!died) {
      ...
      if (day < 5) {
        if (currentScore <= days[day - 1].score) {
          instructions.innerHTML = `<h2>Fin del día 0${day}</h2>Tú puntuación para este día fue: ${currentScore}</p>¡Oh no!, se acabo el tiempo, te tocara iniciar desde el principio.<br> Pero por favor intentalo de nuevo!, Así es la vida!</p>`;
          day = 0;
        } else {
          instructions.innerHTML = `<h2>Fin del día 0${day}</h2>Tú puntuación para este día fue: ${currentScore}</p><p>${days[day].instruction}</p>`;
        }
      } else {
        instructions.innerHTML = `<h2>Haz terminado!, Eres realmente impresionante!</h2><p>Haz atrapado ${fishTracker[0]} valores dorados, ${fishTracker[1]} antivalores morados, ${fishTracker[2]} basuras y ${fishTracker[2]} medusas.<br>Tu resultado final es: ${score} <br> ¿Quieres jugar de nuevo?</p>`;
        day = 0;
      }
    } else {
      day = 0;
      instructions.innerHTML = `<h2>Too bad!</h2><p>Haz sido atrapado por uno!.<br>Se perdio toda tu semana de trabajo!</p>`;
    }
